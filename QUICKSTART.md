...
// Returns: text/event-stream  { type: "message_delta" | "tool_call" | "done", ... }
export const runtime = "nodejs";
export const dynamic = "force-dynamic";
// Full source: routes/agent/route.ts
```

The full route (auth guard, Zod validation, SSE streaming, thread persistence, error handling) lives in [`routes/agent/route.ts`](routes/agent/route.ts) — copy that file rather than this summary.
