...
const root = join(output, 'project'), resources = resolve('dist-mac/Hades.app/Contents/Resources');
...
for(const [name, content] of Object.entries(fixture)) writeFileSync(join(root,name),content);
const baseline = spawnSync(join(resources,'node'), ['--test'], {cwd:root,encoding:'utf8'});
assert.notEqual(baseline.status,0,'Fixture must start with real failing tests');
writeFileSync(join(output,'baseline-tests.txt'),baseline.stdout+baseline.stderr);
let child, lines, events=[], counter=0, pending=new Map(), approvalPolicy='allow', denyCount=0;
const receipts=[];
let activeSession = '';
let checkNumber = 0;
...
function start(){
 child=spawn(join(resources,'node'),[join(resources,'sidecar-entry.js')],{cwd:root,env:{PATH:process.env.PATH,HOME:process.env.HOME,HADES_DATA_DIR:join(output,'data'),HADES_CODEX_HOME:process.env.HADES_CODEX_HOME,HADES_PTY:join(resources,'hades-pty')},stdio:'pipe'});
 ...
 lines.on('line',line=>{const event=JSON.parse(line);events.push(event);appendFileSync(join(output,'events.jsonl'),line+'\n');
  ...
  if(event.kind==='desktop.tool'&&event.status==='done') console.log(JSON.stringify({event:'tool',tool:event.tool,input:event.input?.slice(0,160),result:event.output?.slice(0,200)}));
...
async function stop(){const exited=new Promise(r=>child.once('exit',r));child.stdin.end();await Promise.race([exited,new Promise((_,reject)=>setTimeout(()=>reject(new Error('Sidecar failed to exit')),10000).unref())]);lines.close();}
async function turn(id,input,label){
 ...
 const receipt={label,provider,usage,elapsedMs:Date.now()-started,toolCalls:run.filter(e=>e.kind==='desktop.tool'&&e.status==='done').length,approvals:run.filter(e=>e.kind==='desktop.approval').length,streamed:run.some(e=>e.kind==='desktop.delta'),failure:failure?.message,answer:record.messages.at(-1)?.role === 'assistant' ? record.messages.at(-1).content : undefined};
 receipts.push(receipt);writeFileSync(join(output,'receipts.json'),JSON.stringify({model,output,receipts},null,2));console.log(JSON.stringify({event:'turn',...receipt}));
...
async function verify(label,source){
 ...
 const file=join(output,checkLabel+'.mjs');writeFileSync(file,source);
 const result=spawnSync(join(resources,'node'),[file],{cwd:root,encoding:'utf8',timeout:15000});
 writeFileSync(join(output,checkLabel+'.txt'),result.stdout+result.stderr);
 ...
 writeFileSync(join(output,'receipts.json'),JSON.stringify({model,output,receipts},null,2));console.log(JSON.stringify(receipt));
...
console.log(JSON.stringify({event:'start',model,output}));
try{
 ...
 await stop();writeFileSync(join(output,'receipts.json'),JSON.stringify({model,output,receipts,passed:true},null,2));console.log(JSON.stringify({event:'complete',passed:true,output}));
}catch(e){writeFileSync(join(output,'receipts.json'),JSON.stringify({model,output,receipts,passed:false,error:e.message},null,2));console.error(e);child?.kill();process.exitCode=1;}
