...
## Stable upstream pagination

Orca 54ad550724b2ac05c5884934a2a583aff745b8e5 adds schema4 stable insertion sequences and a persistent random database origin. Opaque cursors bind a hashed session identity, derived stream ID and last sequence. Late/backdated arrivals cannot move earlier pages; duplicate observations do not add sequence entries; conflicts retain separate entries. Reads use one snapshot and refuse orphaned sequence/evidence mappings. A recreated database raises a typed stream-change error, distinct from transport or journal unavailability. Older schema migrations preserve existing evidence; future-schema files stay unchanged/read-only.

workerShow opts into version2 only when usageCursor is supplied (null starts at the beginning). Version1 callers keep the previous shape. This path is local owning-host only; federated/remote support remains pending.

Hades 74a872507d7273d1871a28fa858fc0f0b6979185 decodes both versions, saves each nextCursor with observations and the intent revision in one transaction, and drains at most five pages per refresh. A repeated cursor ends the drain. Truncation remains visible when more pages remain. Reopen resumes from the committed cursor. Only a verified version2 cursor_invalidated response resets the source cursor; prior retained evidence remains. Generic journal/transport failures do not reset it. Version1 hosts remain supported but explicitly lack pagination.

66 Orca source tests across10files and72 Hades tests across6files pass. Targeted Orca module typechecking and full Hades TypeScript checking pass; the provider-bundled sidecar rebuilt. Two cross-repository in-process tests used the actual Orca parser, SQLite storage and worker projection to emit100+5+0+1 late observations, then Hades validated all106 distinct records/cursors/hashes. Legacy version1 also decoded. Producer source hashes match Orca 54ad550724b2ac05c5884934a2a583aff745b8e5. Independent read-only review found no remaining blocker in the scoped pagination changes. Evidence: evidence/orca-usage-pagination-v1.

The upstream100-record collection limit is closed for this local version2 source path. This does not establish complete provider billing: uncorrelated reports, verified aggregation semantics, mandatory provider caps, original Work attempt settlement, remote projection, full pinned fork builds and live/native acceptance remain open. No shipping pin, installed app, registry or remote repository was changed.
