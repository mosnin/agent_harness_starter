...
CI measurement correction: the first continuation revision passed the Mac application build and desktop workflows, but the pre-existing soak assertion failed on a shared Mac runner even in the serial step. Its 3,000-message windows measured about 2.4 ms and 7.1 ms. The test now performs 100,000 real round-trips per window (1.2 million total), retains the same first/last 40% threshold, and has a 30-second bound. Five consecutive local measurements retained 84.7%–93.0% of first-window throughput; no failed sample is retried or excluded by the test. The updated ten-test soak suite passed.
