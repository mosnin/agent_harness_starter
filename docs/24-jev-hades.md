...
- `cat ~/.kube/config`, `file_read ~/.git-credentials`, `cat ~/.docker/config.json`, and `echo pwned > ~/.kube/config` are `target-local` at zero RTT. A warning that only mentions those paths still passes. JWTs / `glpat-` / `xoxe-` are `leaks-secret-local`. `Get-Content …\\NTUSER.DAT` and `file_read …\\System32\\config\\SAM` are the same class. AWS STS / `whsec_` / `xai-` / `dop_v1_` / `shpat_` / `rk_live_` are `leaks-secret-local`.
...
- Desktop `voice.turn` had no size cap (HTTP voice did). Base64 is rejected before decode at the same 8 MiB class; chat/prefetch text is capped at 32k. The sidecar reads capped lines so a huge NDJSON line never reaches `JSON.parse`. AWS STS (`ASIA…`), Stripe `whsec_` / `rk_live_`, `xai-`, `dop_v1_`, and `shpat_` are `leaks-secret-local`. Windows `SAM` / `NTUSER.DAT` are `target-local` the same way `/etc/passwd` is.
...
Assigned tasks were screened; the mesh bus was not. `send` / `broadcast` now replace jailbreak and secret payloads with `{ blocked }` before `onMessage`, and the log drops the oldest line past 200. Convex `deleteThread` pages messages and runs (64 at a time) instead of `.collect()`. `runs:listByThread` defaults to `take(50)`. GitHub `gho_` / `ghu_` / `ghs_` / `ghr_` are the same zero-RTT class as `ghp_`.
...
HTTP voice was capped; the desktop sidecar was not. `voice.turn` now rejects oversized base64 before `Buffer.from`. Chat/prefetch text is 32k, same as `/api/hades`. `readCappedLines` replaces `readline` so a line that never ends in `\\n` cannot grow unbounded. AWS STS (`ASIA`), Stripe webhooks/restricted keys, xAI, DigitalOcean, and Shopify admin tokens are severe. `C:\\Windows\\System32\\config\\SAM` and `NTUSER.DAT` are `target-local`; a warning that only mentions those names still passes.
...
Wave 35/36 sliced lists in HTTP after the adapter loaded everything. `listThreads` now takes `{ limit }` the same way `getMessages` does. Convex lists newest-first via `by_user_and_updated`. A JSON POST that omitted `Content-Length` bypassed the 64 KiB header check and was fully buffered. `readCappedJson` reads at most 64 KiB, then parses. Invalid JSON is 400. MCP and voice do not use it (they must hand the unread `Request` to the transport / multipart parser).
...
`cat /etc/passwd` was `target-local`, but `cat ~/.kube/config`, `~/.git-credentials`, and `~/.docker/config.json` still reached the shell (or waited on Jev). Those paths are the same zero-RTT label, including writes (`echo pwned > ~/.kube/config`). A warning that only *mentions* the path still passes. Pasted JWTs (`eyJ….….…`), GitLab `glpat-`, and Slack `xoxe-` were not in the redact catalog — they now redact and `hasSevereSecret` fail-closes before TypeSafe / Qwen. "JWTs start with eyJ" is not a token.
