...
type TaskDraft = { id: string; title: string; prompt: string; profile: string; dependsOn: string[]; writeMode: "project" | "declared"; writes: string; acceptance: Array<{ id: string; path: string; contains: string }> };
...
reports/summary.md">${esc(task.writes)}</textarea></label><p class="help">Choose declared paths to use this list: one project-relative file or directory per line, no wildcards. An empty declared list means no edits declared. Coordinates edits between tasks; tool permissions still apply.</p>
  ...
  private render() {
    ...
    this.host.querySelectorAll<HTMLElement>("[data-work]").forEach(el => el.onclick = () => { void this.action(el.dataset.work!, el).catch(error => { if (scope === this.scopeVersion) this.fail(error); }); });
    this.host.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>("input, textarea, select").forEach(el => {
      const update = () => {
        if (el.dataset.steering) this.steering[el.dataset.steering] = el.value;
          ...
          else if (["profile", "title", "prompt", "writes"].includes(el.name)) (task as any)[el.name] = el.value;
  ...
  private async action(action: string, el: HTMLElement) {
    if (this.busy && ["message", "run", "stop", "resume"].includes(action)) return;
    const scope = this.scopeVersion;
    ...
    else if (action === "open") { const selected = await this.rpc("work.get", { id: el.dataset.id, profile: this.context!.profile }); if (scope !== this.scopeVersion) return; this.selected = selected; }
