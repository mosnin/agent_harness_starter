...
export class HelmView {
  ...
  private embeddedReview = false;
  async openReview(context: Context, run: Run) {
    this.embeddedReview = true; this.context = context; this.destination = "tasks";
    await this.selectRun(run);
  }
  detachReview() { this.host = undefined; this.selectionVersion++; this.operationVersion++; }
  async open(context: Context) {
    this.embeddedReview = false;
    if (
