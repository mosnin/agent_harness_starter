...
export class MaintenanceView {
  ...
  private render() {
    ...
    const includedOpen = this.host.querySelector<HTMLDetailsElement>("#maintenance-included")?.open;
    this.host.innerHTML = `<div class="page maintenance-page"><div class="page-heading"><h1>Maintenance</h1><p>Keep a private backup and inspect local runtime health.</p></div>
      ...
      <label class="field">Backup file<input id="maintenance-backup-path" name="backupPath" autocorrect="off" autocapitalize="off" spellcheck="false" type="text" value="${esc(this.path)}" placeholder="/path/to/Hades-backup.hades-backup.json" ${this.busy ? "disabled" : ""}></label>
      ...
      ${this.verified ? `<p role="status">Verified ${esc(this.verified.files.length)} files · ${esc(size(this.verified.totalBytes))}</p><p class="help">${esc(this.verified.scope)}</p><details id="maintenance-included"><summary id="maintenance-included-summary">Included files</summary>${this.verified.files.map((file: Row) => `<p class="help">${esc(file.path)} · ${esc(size(file.bytes))}</p>`).join("")}</details>` : ""}</section>
    ...
    this.host.querySelectorAll<HTMLElement>("[data-maintenance]").forEach((element) => {
      element.id = `maintenance-action-${element.dataset.maintenance}-${element.dataset.index ?? ""}`;
    });
    const included = this.host.querySelector<HTMLDetailsElement>("#maintenance-included");
    if (included && includedOpen !== undefined) included.open = includedOpen;
  ...
  private async action(action: string, index: number) {
    ...
    try {
      ...
      if (this.host && document.activeElement === document.body) restoreFocus(this.host, focus);
    }
  }
}
