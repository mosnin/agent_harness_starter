...
export class HelmOrcaView {
  ...
  private render() {
    ...
    for (const field of ["prompt","agent","model"]) {
      ...
      if (el) { const update = () => { if (!s.pending) (s as any)[field] = el.value; }; el.oninput = update; el.onchange = update; }
    }
  ...
  private async start() {
    ...
    if (state.prompt.length > 20000 || state.prompt.includes(String.fromCharCode(0)) || state.model.trim().startsWith("-") || state.model.trim().length > 200 || !["claude", "codex", "opencode"].includes(state.agent)) { this.error = "Check the task, agent and model preference before starting."; this.render(); return; }
    state.pending ??= { requestId: crypto.randomUUID(),prompt: state.prompt.trim(),agent: state.agent,...(state.model.trim() ? {model:state.model.trim()} : {}) };
