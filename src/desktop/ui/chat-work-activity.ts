...
export class ChatWorkActivity {
  ...
  private render(){
    ...
    this.host.innerHTML=`${this.error?`<p role="alert">${esc(this.error)} <button type="button" data-chat-work="retry">Try again</button></p>`:""}${this.rows.length?`<section class="chat-work-progress" aria-label="Work in this conversation"><h3>Working with Hades</h3>${this.rows.map(({kind,value:r})=>`<article class="chat-work-card"><div><strong>${esc(r.title||r.objective||r.prompt?.slice(0,120)||"Task")}</strong><span>${esc(label(r.status))}</span></div>${r.error?`<p>${esc(r.error)}</p>`:""}<details data-task="${esc(r.id)}" ${this.openDetails.has(r.id)?"open":""}><summary>Progress and result</summary>${kind==="work"?(r.tasks??[]).map((task:Row)=>`<section><strong>${esc(task.title||task.prompt||task.id)}</strong><p>${esc(label(task.status))}</p>${task.error?`<p role="alert">${esc(task.error)}</p>`:""}${task.evidence?.length?`<ul>${task.evidence.map((e:Row)=>`<li>${esc(e.path)}</li>`).join("")}</ul>`:""}${task.answer?`<pre>${esc(String(task.answer).slice(-12000))}</pre>`:""}</section>`).join(""):`<pre>${esc(String(r.output||"No output yet.").slice(-12000))}</pre>`}</details><div class="chat-work-actions">${kind==="orca"?`<button type="button" data-chat-work="read" data-id="${esc(r.id)}">Read output</button>`:kind==="helm"?`<button type="button" data-chat-work="review" data-id="${esc(r.id)}">Review changes</button>`:`<button type="button" data-chat-work="discuss" data-id="${esc(r.id)}">Discuss results</button>`}${["running","starting","stopping","ready","unknown"].includes(r.status)?`<button type="button" data-chat-work="stop" data-id="${esc(r.id)}" ${this.busy.has(r.id)?"disabled":""}>${this.busy.has(r.id)?"Requesting stop…":"Stop task"}</button>`:""}</div></article>`).join("")}<button type="button" data-chat-work="refresh">Refresh progress</button></section>`:""}<div class="chat-work-review-host"></div>`;
