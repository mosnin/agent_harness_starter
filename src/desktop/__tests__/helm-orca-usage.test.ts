...
import {
  decodeHelmOrcaUsage,
  type HelmOrcaUsageObservation,
} from "../core/helm-orca-usage";
...
describe("Orca usage projection decoder", () => {
  it("validates canonical observations without conflating client-operation and worker dispatch", () => {
    ...
    expect(decoded.observations[0].observation.identity.dispatchId).toBe(
      "client-operation",
    );
  ...
  it("distinguishes missing, unavailable, malformed and wrong authority", () => {
    expect(decodeHelmOrcaUsage(undefined, expected)).toEqual({
      state: "unsupported",
    });
    expect(decodeHelmOrcaUsage(null, expected).state).toBe("malformed");
    expect(
      decodeHelmOrcaUsage({ ...fixture(), dispatchId: "foreign" }, expected)
        .state,
    ).toBe("mismatch");
    expect(
      decodeHelmOrcaUsage({ ...fixture(), sessionId: "foreign" }, expected)
        .state,
    ).toBe("mismatch");
    for (const reason of [
      "identity_unproven",
  ...
  it.each([
    ...
    "4",
    undefined,
  ])("rejects invalid token count %s", (value) => {
    const page = fixture();
  ...
  it("rejects nested foreign sessions even when recomputed hashes agree", () => {
    ...
    const o = {
      ...
      identity: {
        ...p.observations[0].observation.identity,
        sessionId: "foreign",
      },
    };
  ...
  it("bounds rows, rejects duplicates, and preserves conflicts and truncation without totals", () => {
    ...
    const c = fixture();
    c.observations.push(
      row({ ...c.observations[0].observation, inputTokens: 8 }),
    );
...
it("accepts opt-in cursor evidence and refuses malformed or missing cursors", () => {
  const value = { ...fixture(), version: 2, nextCursor: "opaque_cursor" };
  expect(decodeHelmOrcaUsage(value, expected)).toEqual(value);
  for (const nextCursor of [undefined, "", "a".repeat(2049), "bad cursor", -1])
    expect(
      decodeHelmOrcaUsage({ ...value, nextCursor }, expected),
    ).toMatchObject({ state: "malformed" });
  expect(decodeHelmOrcaUsage({ ...value, version: 1 }, expected)).toMatchObject(
    { state: "malformed" },
  );
});
it("only version2 advertises cursor invalidation", () => {
  const value = {
    version: 2,
    dispatchId: expected.dispatchId,
    state: "unavailable",
  ...
  expect(decodeHelmOrcaUsage(value, expected)).toMatchObject({
    state: "unavailable",
    reason: "cursor_invalidated",
  });
  expect(decodeHelmOrcaUsage({ ...value, version: 1 }, expected)).toMatchObject(
