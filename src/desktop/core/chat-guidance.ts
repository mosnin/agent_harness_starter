export function chatGuidance(names: Iterable<string>): string {
  ...
  if (available.has("delegation_accept_result")) guidance.push("For completed read-only team research without file checks, inspect the reports and propose delegation_accept_result with the current resultDigest and a concrete review summary. Keep review in this conversation; do not invent file outputs to finish a research task. This action cannot accept coding tasks, failed checks or unfinished workers.");
