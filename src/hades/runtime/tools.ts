...
export function workspaceTools(root: string, shellCommands: string[] = []): ToolRegistry {
  const tools = builtinRegistry();
  tools.register(createFileOpsTool({ root }).tool);
  if (shellCommands.length) tools.register(createShellTool({ cwd: root, policy: { allowedCommands: shellCommands, timeoutMs: 30_000, maxOutputBytes: 16_384 } }).tool);
  return tools;
}
